...
! o vetor a é igual a soma de dois valores do vetor b.
	USE MPI
		...
		INTEGER :: n
		REAL :: dx
		REAL, DIMENSION(:), ALLOCATABLE :: a, b, a_aux, W
...
! Initializing MPI
		...
		CALL MPI_ISEND(b(iend), 1, MPI_REAL, inext, 1, MPI_COMM_WORLD, isend1, ierr)
		CALL MPI_ISEND(b(ista), 1, MPI_REAL, iprev, 1, MPI_COMM_WORLD, isend2, ierr)
		CALL MPI_ISEND(b(ista+1),1,MPI_REAL, iprev, 1, MPI_COMM_WORLD, isend3, ierr)
		CALL MPI_ISEND(b(iend-1), 1, MPI_REAL, inext, 1, MPI_COMM_WORLD, isend4, ierr)

		CALL MPI_IRECV(b(ista-1), 1, MPI_REAL, iprev, 1, MPI_COMM_WORLD, irecv1, ierr)
		CALL MPI_IRECV(b(iend+1), 1, MPI_REAL, inext, 1, MPI_COMM_WORLD, irecv2, ierr)
		CALL MPI_IRECV(b(ista-2), 1, MPI_REAL, iprev, 1, MPI_COMM_WORLD, irecv3, ierr)
		CALL MPI_IRECV(b(iend+2), 1, MPI_REAL, inext, 1, MPI_COMM_WORLD, irecv4, ierr)
		...
		DO i = ista2,iend2
			a_aux(i) = (w(1)*b(i-2) +  w(2)*b(i-1) +  w(3)*b(i) +  w(2)*b(i+1) +  w(1)*b(i+2))/(dx**2)
		END DO
...
STOP
END PROGRAM dif_finitas_2ordem
