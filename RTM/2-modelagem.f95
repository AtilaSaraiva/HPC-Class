PROGRAM MODELAGEM
        ...
        !       call mpi_sendrecv(p(1,jend),nz,MPI_REAL,inext,0,p(1,jsta-1),&
        !       nz,MPI_REAL,iprev,MPI_ANY_TAG,MPI_COMM_WORLD,isend1,ierr)
        !       call mpi_sendrecv(p(1,jsta),nz,MPI_REAL,iprev,0,p(1,jend+1),&
        !       nz,MPI_REAL,inext,MPI_ANY_TAG,MPI_COMM_WORLD,isend2,ierr)
        !       call mpi_sendrecv(p(1,jend-1),nz,MPI_REAL,inext,0,p(1,jsta-2),&
        !       nz,MPI_REAL,iprev,MPI_ANY_TAG,MPI_COMM_WORLD,isend3,ierr)
        !       call mpi_sendrecv(p(1,jsta+1),nz,MPI_REAL,iprev,0,p(1,jend+2),&
        !       nz,MPI_REAL,inext,MPI_ANY_TAG,MPI_COMM_WORLD,isend4,ierr)
                
                call mpi_sendrecv(p(1,jend),nz,MPI_REAL,inext,0,&
                                  p(1,jsta-1),nz,MPI_REAL,iprev,0,MPI_COMM_WORLD,status,ierr)

                call mpi_sendrecv(p(1,jend-1),nz,MPI_REAL,inext,1,&
                                  p(1,jsta-2),nz,MPI_REAL,iprev,1,MPI_COMM_WORLD,status,ierr)

                call mpi_sendrecv(p(1,jsta),nz,MPI_REAL,iprev,2,&
                                  p(1,jend+1),nz,MPI_REAL,inext,2,MPI_COMM_WORLD,status,ierr)

                call mpi_sendrecv(p(1,jsta+1),nz,MPI_REAL,iprev,3,&
                                  p(1,jend+2),nz,MPI_REAL,inext,3,MPI_COMM_WORLD,status,ierr)

       ! call mpi_sendrecv(p(1,jend-1), 2*nz, MPI_REAL, inext, 1, &
       !                   p(1,jsta-2), 2*nz, MPI_REAL, iprev,    &
       !                   1, MPI_COMM_WORLD, status, ierr)
       ! call mpi_sendrecv(p(1,jsta  ), 2*nz, MPI_REAL, iprev, 2, &
       !                   p(1,jend+1), 2*nz, MPI_REAL, inext,    &
...
lap(iz,ix)=(-2.5*p(iz,ix)+4./3.*(p(iz+1,ix) + p(iz-1,ix))-&
                1./12.*(p(iz+2,ix) + p(iz-2,ix)))*dz2inv
lap(iz,ix)= lap(iz,ix)+(-2.5*p(iz,ix)+4./3.*(p(iz,ix+1) +&
                 p(iz,ix-1))-1./12.*(p(iz,ix+2) + p(iz,ix-2)))*dx2inv                          
