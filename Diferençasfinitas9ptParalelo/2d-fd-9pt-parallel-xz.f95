PROGRAM main
    ...
    CALL MPI_ISEND(works1(jjsta),jlen,MPI_REAL8,inext,1, &
    MPI_COMM_WORLD,jsend1,ierr)
    CALL MPI_ISEND(works2(jjsta),jlen,MPI_REAL8,iprev,1, &
    MPI_COMM_WORLD,jsend2,ierr)
    CALL MPI_IRECV(workr1(jjsta),jlen,MPI_REAL8,iprev,1, &
    MPI_COMM_WORLD,jrecv1,ierr)
    CALL MPI_IRECV(workr2(jjsta),jlen,MPI_REAL8,inext,1, &
    ...
    DO j = jsta2, jend1
        DO i = ista2, iend1
            b(i,j) = a(i-1,j ) + a(i, j-1) +  a(i, j+1) + a(i+1,j ) + a(i-1,j-1) &
            + a(i+1,j-1) + a(i-1,j+1) + a(i+1,j+1)
        ENDDO
    ENDDO


    call MPI_ALLREDUCE ( b, c, m*n, MPI_REAL8, MPI_SUM, &
    MPI_COMM_WORLD,ierr)

    IF (myrank == 0)  then
        open (unit=9, file='result-9-pt-parallel.dat', ACCESS='direct', &
